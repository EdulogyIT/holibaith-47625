  ...
  return (
    <section className="px-4 py-6 bg-white">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold">{t('exploreByCities')}</h2>
        <button 
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
        {cities.map((city) => (
          <div
            ...
            className="relative h-36 w-64 flex-shrink-0 rounded-2xl overflow-hidden shadow-sm cursor-pointer"
          >
            <img
              src={city.image}
              alt={city.name}
            ...
            <div className="absolute bottom-3 left-3 text-white">
              ...
              <p className="text-xs">{city.properties} {t('properties')}</p>
            </div>
          </div>
        ))}
      </div>
